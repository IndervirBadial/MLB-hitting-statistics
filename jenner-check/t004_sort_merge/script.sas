...
*Merge the datasets with clusters and the dataset with factors -- demonstrated
 here on BASEBALL joined against a small mock cluster-assignment table, since
 the real cluster/factor pipeline (PROC FACTOR OUT=, PROC CLUSTER, PROC TREE)
 hits a separate Jenner PROC FACTOR OUT= issue with this repo's N<P shape;
Proc Sort data=BASEBALL;
By full_name; run;

data clusters;
  length full_name $41 cluster 8;
  input full_name $ 1-20 cluster;
  datalines;
Aaron Judge         1
Shohei Ohtani       1
Freddie Freeman     2
...
data baseballclust;
merge BASEBALL clusters;
by full_name;
run;

proc print data=baseballclust;
  var full_name cluster AvergaeExitVelocity BarrelBattedRate HardHitPct;
run;
